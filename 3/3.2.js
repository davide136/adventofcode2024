...
for(let i = 0; i < testo.length; i++){
    let temp = testo.substring(i);
    let index = temp.indexOf(markerStart);
    if(index != -1){
...
iterable.forEach(p => {
    if(p.type == "dont"){
        console.log("stopping from " + p.pos)
        doStuff = false;
    }
    else if(p.type == "do"){
        console.log("starting from " + p.pos)
        doStuff = true;
    }
    
    if(doStuff){
        let position = p.pos;
        let pezzo = testo.substring(position, position + markerStart.length + markerEnd.length + separator.length + 6 )
        let indexEnd = pezzo.indexOf(markerEnd);
        if(indexEnd != -1){
            pezzo = pezzo.substring(markerStart.length, indexEnd)     
            let indexSep = pezzo.indexOf(separator);
            if(indexSep > 0){
                let leftNum = +pezzo.substring(0, indexSep)
                let rightNum = +pezzo.substring(indexSep+1)
                
                if(leftNum != NaN && rightNum != NaN){
                    console.log("multiplying " + pezzo + " values: " + leftNum + " * " + rightNum + " res = " + leftNum*rightNum + " with sum " + sum)
